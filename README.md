...
#### Solución implementada
Se modificaron los clientes y el servidor para implementar el proceso completo de notificación de fin de apuestas, realización del sorteo y consulta de ganadores. Los cambios incluyeron:

##### Cliente:
1. Implementación del método `notifyFinishedSendingBets()` para informar al servidor que la agencia ha terminado de enviar todas sus apuestas
2. Desarrollo del método `getLotteryWinners()` para:
   - Consultar periódicamente al servidor por los ganadores del sorteo
   - Esperar hasta que el sorteo se haya realizado
   - Mostrar en el log la cantidad de ganadores correspondientes a la agencia

##### Servidor:
1. Añadido de estado interno para:
   - Registrar qué agencias han finalizado el envío de apuestas mediante el set `_finished_agencies`
   - Almacenar las apuestas ganadoras por agencia en `_winning_bets_by_agency`
   - Indicar si el sorteo ya fue realizado con `_lottery_done`
2. Implementación del método `_do_lottery()` que:
   - Se ejecuta cuando todas las agencias han finalizado
   - Utiliza `load_bets()` para cargar todas las apuestas
   - Identifica ganadores con `has_won()` y los clasifica por agencia
   - Registra el evento de sorteo exitoso en el log
3. Añadido de manejadores para los nuevos tipos de mensaje:
   - `FinishedSendingBetsMessage`: Para registrar que una agencia ha terminado de enviar sus apuestas
   - `LotteryWinnersRequestMessage`: Para responder consultas sobre ganadores

##### Extensión del protocolo:
1. Nuevos tipos de mensaje:
   - `FINISHED_SENDING_BETS` (3): Notificación de fin de envío de apuestas
   - `LOTTERY_WINNERS_REQUEST` (4): Solicitud de lista de ganadores
   - `LOTTERY_WINNERS_RESPONSE` (5): Respuesta con los DNIs ganadores o estado no listo
2. Formatos de mensaje:
   - `FINISHED_SENDING_BETS`: ID de agencia (4 bytes)
   - `LOTTERY_WINNERS_REQUEST`: ID de agencia (4 bytes) 
   - `LOTTERY_WINNERS_RESPONSE`: Estado (1 byte) + Lista de DNIs (8 bytes cada uno)

Notas:
- El servidor espera la notificación de las 5 agencias antes de realizar el sorteo
- Si un cliente consulta por ganadores antes del sorteo, el servidor responderá con el estado "no listo"
- Cada agencia solo recibe información sobre sus propios ganadores
...
3. Verificar los logs para seguir el flujo completo:
```bash
make docker-compose-logs
```
...
Se deberían ver mensajes como:
```
client1 | action: notificar_fin_apuestas | result: success | client_id: 1
client2 | action: notificar_fin_apuestas | result: success | client_id: 2
...
